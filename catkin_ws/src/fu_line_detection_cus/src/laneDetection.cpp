...
static const uint8_t MY_ROS_QUEUE_SIZE = 1;
    ...
    : nh_(nh), priv_nh_("~")
{
	...
	priv_nh_.param<double>(node_name+"/im2ws",im2ws, 0.00125);
	priv_nh_.param<double>(node_name+"/ws_offset",ws_offset, 0.4);
	
	priv_nh_.param<bool>(node_name+"/invertedColors", invertedColors, false);
    ...
    read_images_ = nh.subscribe("/rgb/image_raw", MY_ROS_QUEUE_SIZE, &cLaneDetectionFu::ProcessInput,this);
	...
	//publish_pcl_lane = nh.advertise<PointCloud>("/lane_model/pcl_lane", MY_ROS_QUEUE_SIZE);	
	publish_pcl_lane = nh.advertise<sensor_msgs::PointCloud2>("lane_model/pcl_lane", MY_ROS_QUEUE_SIZE);
	...
	//Four points in the original image are defined as well as their position in a repojected image. 
	//The homography is calcualted just once. 
	#ifdef HOMOGRAPHY
		//Source points in the original image.
		pts_src.push_back(cv::Point2f(162,289));
		pts_src.push_back(cv::Point2f(475,289));
		pts_src.push_back(cv::Point2f(638,433));
		pts_src.push_back(cv::Point2f(0,433));

		//Desired position for the source points in the reprojected image.
		//This poinst are used ifwe want to process a larger reprojection
		pts_dst.push_back(cv::Point2f(75,149));
		pts_dst.push_back(cv::Point2f(124,149));
		pts_dst.push_back(cv::Point2f(124,199));
		pts_dst.push_back(cv::Point2f(75,199));
		
		//This poinst are used ifwe want to process an smaller reprojection.
		/*pts_dst.push_back(cv::Point2f(50,100));
		pts_dst.push_back(cv::Point2f(149,100));
		pts_dst.push_back(cv::Point2f(149,199));
		pts_dst.push_back(cv::Point2f(50,199));*/
		
		//The parameters should be changed accordingly if the desired points are chaged.
...
{        	
...
#ifdef HOMOGRAPHY
	//The homography is applied to the incoming image. 
    Mat transformedImage;
	Mat remapped_image;
	cv::warpPerspective(image, transformedImage, h, cv::Size(200,200), cv::INTER_LINEAR,cv::BORDER_CONSTANT,Scalar(255,255,255));	
#ifdef PAINT_OUTPUT
	cv::imshow("Cut", image);
    ...
    //The edges are found using a Scharr filter.
    vector<vector<EdgePoint>> edges = cLaneDetectionFu::scanImageScharr(transformedImage);	
    ...
    //edges -> lane markings
    //We define a two functions to extract lane markings.
	//extractLaneMarkingsInverse is used when the lanes have a darker color than the background. 
	//extractLaneMarkingsInverse is used when the lanes have a lighter color than the background.	
	vector<FuPoint<int>> laneMarkings;
	if(invertedColors){
		laneMarkings = cLaneDetectionFu::extractLaneMarkingsInverse(edges);
	}else{
		laneMarkings = cLaneDetectionFu::extractLaneMarkings(edges);
	}
	cout<<"Lane detector: marcas: "<<laneMarkings.size()<<endl;
	...
	for(int i = 0; i < (int)laneMarkings.size(); i++)
        {         
            FuPoint<int> marking = laneMarkings[i];
            cv::Point markingLoc = cv::Point(marking.getX(), marking.getY());
            cv::circle(transformedImagePaintable,markingLoc,1,cv::Scalar(255,255,255),-1);         
        }
	...
	pcl_cloud.header.frame_id = "auto_car_model";
	pcl_cloud.height = 1;
    ...
    //---------------------- DEBUG OUTPUT RANSAC POLYNOMIALS ---------------------------------//
        ...
        for(int i = minYPolyRoi; i < maxYRoi; i++)
        {
            ...
            cv::circle(transformedImagePaintableRansac,pointLocLeft,2,cv::Scalar(0,0,255),-1);
            cv::Point pointLocCenter = cv::Point(polyCenter.at(i), i);
            cv::circle(transformedImagePaintableRansac,pointLocCenter,2,cv::Scalar(0,255,0),-1);
            cv::Point pointLocRight = cv::Point(polyRight.at(i), i);
            cv::circle(transformedImagePaintableRansac,pointLocRight,2,cv::Scalar(255,0,0),-1);
    ...
    {         
        FuPoint<int> marking = laneMarkings[i];
        cv::Point markingLoc = cv::Point(marking.getX(), marking.getY());
		cv::circle(transformedImagePaintableLaneModel,markingLoc,1,cv::Scalar(0,255,0),-1);         
    }*/
    ...
    if (lanePolynomial.hasDetected()) {
        ...
        int codeLine = -1;
		if(lanePolynomial.getLastUsedPosition() == LEFT){
			codeLine = 2;
		}else if(lanePolynomial.getLastUsedPosition() == CENTER){
			codeLine = 1;
		}else{
			codeLine = 0;
		}
        
        for(int i = minYPolyRoi; i < maxYRoi; i=i+1)
        {
            cv::Point pointLoc = cv::Point(lanePolynomial.getLanePoly().at(i)+proj_image_w_half, i);
            cv::circle(transformedImagePaintableLaneModel,pointLoc,2,cv::Scalar(b,g,r),-1);
			
			//Conversion of the points in pixels to the workspace of the robot.
			float x = pointLoc.x - (transformedImage.cols/2);
			x = x*im2ws;			
			float y = transformedImage.rows - pointLoc.y;
			y = y*im2ws + ws_offset; 								
			float z = codeLine;			
			pcl::PointXYZ p;
			p.x = y; //Interchange the values for rotating the point -90 degrees. This way the X axis from the points  is the X axis of the car reference frame.
			p.y = -x;
			p.z = z;						
	...
	//cout<<"Tamaño: "<<pcl_cloud.height<<" x "<< pcl_cloud.width<<"  "<<pcl_cloud.size()<<endl;
	sensor_msgs::PointCloud2 msg_pc;
	pcl::toROSMsg<pcl::PointXYZ>(pcl_cloud,msg_pc);
	publish_pcl_lane.publish(msg_pc);
	
	vector<double> cs = lanePolynomial.getLanePoly().getCoefficients();
	...
	double duration = double(t2-t1)/cv::getTickFrequency();
	ROS_INFO_STREAM("Processing time: "<<duration<<" s");
#endif
}
...
vector<vector<LineSegment<int>> > cLaneDetectionFu::getScanlines() {
	...
	ROS_INFO_STREAM("Contorno");
	cout<<cv::Point(proj_image_w_half-(roi_bottom_w/2),maxYRoi-1)<<endl;
	cout<<cv::Point(proj_image_w_half+(roi_bottom_w/2),maxYRoi-1)<<endl;
	cout<<cv::Point(proj_image_w_half+(roi_top_w/2),minYPolyRoi)<<endl;
	cout<<cv::Point(proj_image_w_half-(roi_top_w/2),minYPolyRoi)<<endl;
...
/**
 * Walk with Scharr kernel along all scanlines.
 * @param image
 * @return All edgePoints on side, sorted by scanlines.
 */
vector<vector<EdgePoint>> cLaneDetectionFu::scanImageScharr(cv::Mat image) {
    //ROS_INFO_STREAM("scanImage() - " << scanlines.size() << " scanlines.");
    vector<vector<EdgePoint>> edgePoints;
    ...
    for (auto scanline : scanlines) {
        ...
        for (auto segment : scanline) {         
            ...
            for (int i = start; i < end - g_kernel1DWidth; i++) {
                ...
                //cv::Mat uses ROW-major system -> .at(y,x)
                // use kernel width 5 and try sobel kernel
                sum -= 2*image.at<uint8_t>(offset-1, i);
                sum -= 2*image.at<uint8_t>(offset-1, i+1);
                // kernel is 0
                sum += 2*image.at<uint8_t>(offset-1, i+3); //2
                sum += 2*image.at<uint8_t>(offset-1, i+4);

                sum -= 3*image.at<uint8_t>(offset, i);
                sum -= 6*image.at<uint8_t>(offset, i+1);
                // kernel is 0
                sum += 6*image.at<uint8_t>(offset, i+3);//2
                sum += 3*image.at<uint8_t>(offset, i+4);
...
std::vector<FuPoint<int>> cLaneDetectionFu::extractLaneMarkings(const std::vector<std::vector<EdgePoint>>& edges) {
    ...
    for (const auto& line : edges) {
        ...
        for (
			...
			//cout<<"Edge: "<<edgePosition->getImgPos()<<"\t"<<edgePosition->getValue()<<endl;
			//cout<<"Next Edge: "<<nextEdgePosition->getImgPos()<<"\t"<<nextEdgePosition->getValue()<<endl;
			//cout<<"Edge Positive?: "<<edgePosition->isPositive()<<endl;
			//cout<<"Next Edge Positive?: "<<nextEdgePosition->isPositive()<<endl;
            if (edgePosition->isPositive() and not nextEdgePosition->isPositive()) {
				...
				//cout<<"Distancia: "<<(candidateStartEdge - candidateEndEdge).squaredMagnitude()<<endl;
                if ((candidateStartEdge - candidateEndEdge).squaredMagnitude() < laneMarkingSquaredThreshold) {
					...
					//cout<<"Edge center: "<<result.back()<<endl;;
                }
            }
        }
    }
...
//uses Edges to extract lane markings. This is used when we have a light background with dark lines.
std::vector<FuPoint<int>> cLaneDetectionFu::extractLaneMarkingsInverse(const std::vector<std::vector<EdgePoint>>& edges) {
    vector<FuPoint<int>> result;
    ...
    for (const auto& line : edges) {
        ...
        for (
			...
			//cout<<"Edge Positive?: "<<edgePosition->isPositive()<<endl;
			//cout<<"Next Edge Positive?: "<<nextEdgePosition->isPositive()<<endl;
            if (nextEdgePosition->isPositive() and not edgePosition->isPositive()) {
                FuPoint<int> candidateStartEdge = edgePosition->getImgPos();
                FuPoint<int> candidateEndEdge = nextEdgePosition->getImgPos();
				//cout<<"Distancia: "<<(candidateStartEdge - candidateEndEdge).squaredMagnitude()<<endl;
                if ((candidateStartEdge - candidateEndEdge).squaredMagnitude() < laneMarkingSquaredThreshold) {
...
bool cLaneDetectionFu::isInDefaultRoi(ePosition position, FuPoint<int> &p, vector<FuPoint<int>> &points) {
    if (p.getY() < minYDefaultRoi || p.getY() > maxYRoi) {
        return false;
    }
    else{ 
		if(!points.empty()){			
			if (horizDistanceToDefaultLine(position, p) <= interestDistanceDefault and 
				horizDistance(p, points.back()) <= interestDistancePoly){
				return true;				
			}else{
				return false;
			}
		}else{
			if (horizDistanceToDefaultLine(position, p) <= interestDistanceDefault){
				return true;				
			}else{
				return false;
			}
...
bool cLaneDetectionFu::isInPolyRoi(NewtonPolynomial& poly, FuPoint< int >& p, vector< FuPoint< int > >& points)
{
	if (p.getY() < minYPolyRoi || p.getY() > maxYRoi) {
        return false;
    }
...
//Mean value of lanes 
float cLaneDetectionFu::meanValueLane(vector<FuPoint<int>> & lanetest)
{
  float mean_y=0.0;
  FuPoint<int> lanepoint1, lanepoint2;
  for (int i=0;i<lanetest.size()-1;i++)
  {
    lanepoint1=lanetest[i];
    lanepoint2=lanetest[i+1];
    mean_y=mean_y+abs(lanepoint1.getY()-lanepoint2.getY());
        
  }
  mean_y=mean_y/lanetest.size();
  return mean_y;
}
...
bool cLaneDetectionFu::ransacInternal(ePosition position,
	...
	//cout<<"Ransac"<<endl;
	//cout<<position<<"\t"<<laneMarkings.size()<<endl;
    if (laneMarkings.size() < 7) {
		//cout<<"Pocos marcadores"<<endl;
        return false;
    }
 ...
 */
//We have extended the horizontal distance to default line because when the lane was near to horizontal line it was not detected
bool cLaneDetectionFu::polyValid(ePosition position, NewtonPolynomial poly,
    ...
    if (horizDistanceToDefaultLine(position, p1) > 40) {
        //ROS_INFO_STREAM("Poly was to far away from default line at y = "<<polyY1);
        return false;
    }

    FuPoint<int> p2 = FuPoint<int>(poly.at(polyY2), polyY2);    //y = 60

    if (horizDistanceToDefaultLine(position, p2) > 60) {
        //ROS_INFO_STREAM("Poly was to far away from default line at y = "<<polyY2);
        return false;
    }

    FuPoint<int> p3 = FuPoint<int>(poly.at(polyY3), polyY3);    //y = 40

    if (horizDistanceToDefaultLine(position, p3) > 80) {
        //ROS_INFO_STREAM("Poly was to far away from default line at y = "<<polyY3);
        return false;
    }
    ...
    if (prevPoly.getDegree() != -1) {
        ...
        if (horizDistance(p4, p5) > 50) {//0.05 * meters) {
            //ROS_INFO_STREAM("Poly was to far away from previous poly at y = "<<polyY1);
            return false;
        }
        ...
        if (horizDistance(p6, p7) > 80) {//0.05 * meters) {
            //ROS_INFO_STREAM("Poly was to far away from previous poly at y = "<<polyY2);
            return false;
        }
    }
...
void cLaneDetectionFu::config_callback(line_detection_fu::LaneDetectionConfig &config, uint32_t level) {
	...
	ROS_INFO_STREAM("Entrando a scanlines");
